...
SELECT cursus, begindatum, (
    ...
    WHERE ins.begindatum = uitv.begindatum
	AND ins.cursus = uitv.cursus)
...
SELECT trainer.voorl, trainer.naam
FROM medewerkers trainer, inschrijvingen ins
WHERE trainer.chef IN (
    SELECT cursist
    FROM  uitvoeringen uitv
    WHERE uitv.cursus = ins.cursus
	AND trainer.mnr = uitv.docent
  	AND cursus IN (
  		SELECT code
  		FROM cursussen cursus
  		WHERE type = 'ALG'
  	)
);
