...
FROM medewerkers
GROUP BY mnr
HAVING (functie = 'TRAINER' OR functie = 'VERKOPER' )
AND EXTRACT(YEAR FROM gbdatum) < 1980;
...
SELECT cursus, begindatum, COUNT(inschrijvingen) AS aantal_inschrijvingen
FROM inschrijvingen
GROUP BY cursus, begindatum
HAVING COUNT(inschrijvingen) >= 3
AND EXTRACT(YEAR FROM begindatum) = 2019;
...
SELECT cursist, COUNT(cursist) cursist_cursus, cursus
FROM inschrijvingen
...
SELECT MAX(EXTRACT(YEAR FROM gbdatum)) - MIN(EXTRACT( YEAR FROM gbdatum)) AS verschil,
EXTRACT(YEAR FROM CURRENT_DATE) - AVG( EXTRACT(YEAR FROM gbdatum) ) AS gemiddeld
FROM medewerkers;
